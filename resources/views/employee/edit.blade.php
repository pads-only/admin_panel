            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4 ">
                <form action="{{route('employees.update', $employee->id)}}" method="post" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="first_name" class="block mt-1 w-96" type="text" name="first_name" value="{{$employee->first_name}}" autofocus autocomplete="username" />
                        <x-input-error :messages="$errors->get('first_name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="last_name" :value="__('Last Name')" />
                        <x-text-input id="last_name" class="block mt-1 w-96" type="text" name="last_name" value="{{$employee->last_name}}"/>
                        <x-input-error :messages="$errors->get('last_name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="email" :value="__('Email')" />
                        <x-text-input id="email" class="block mt-1 w-96" type="email" name="email" value="{{$employee->email}}"/>
                    ...
                    <div>
                        ...
                        <select name="companies_id" id="companies" class="w-96 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            ...
                            @foreach ($companies as $company)
                                <option value="{{$company->id}}" {{ $employee->companies_id === $company->id ? 'selected' : '' }}>{{$company->name}}</option>
                            @endforeach
                        </select>
                     ...
                     <div>
                        ...
                        <x-text-input id="phone" class="block mt-1 w-96" type="number" name="phone" value="{{$employee->phone}}"/>
